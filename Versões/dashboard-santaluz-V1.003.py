...
if page == "Prestação de Contas Mensal":
    ...
    with colA:
        ...
        if not df_receitas.empty:
            ...
            html_t += "<tr style='border-bottom: 2px solid #aaa;'><th style='padding: 8px;'>Categoria</th><th style='padding: 8px; text-align:right;'>Porcentagem</th><th style='padding: 8px; width: 150px;'>Valor</th></tr>"
            for _, row in df_rec_table.iterrows():
                ...
                html_t += f"<tr style='border-bottom: 1px solid #ddd;'><td style='padding: 8px;'>{row['category_name']}</td><td style='padding: 8px; text-align:right;'>{pct}</td><td style='padding: 8px;'><div style='display: flex; justify-content: space-between; width: 115px; margin-left: auto;'><span>R$</span><span style='white-space: nowrap;'>{val_str}</span></div></td></tr>"
            total_val_str = f"{total_receitas:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
            html_t += f"<tr style='font-weight: bold; color: #27ae60; border-top: 2px solid #aaa;'><td style='padding: 8px;'>TOTAL RECEITAS</td><td style='padding: 8px; text-align:right;'>100.00%</td><td style='padding: 8px;'><div style='display: flex; justify-content: space-between; width: 115px; margin-left: auto;'><span>R$</span><span style='white-space: nowrap;'>{total_val_str}</span></div></td></tr>"
            html_t += "</table></div>"
    ...
    with colB:
        ...
        if not df_despesas.empty:
            ...
            html_t += f"<tr style='font-weight: bold; color: #c0392b; border-top: 2px solid #aaa;'><td style='padding: 8px;'>TOTAL DESPESAS</td><td style='padding: 8px; text-align:right;'>100.00%</td><td style='padding: 8px;'><div style='display: flex; justify-content: space-between; width: 115px; margin-left: auto;'><span>R$</span><span style='white-space: nowrap;'>{total_val_str}</span></div></td></tr>"
            html_t += "</table></div>"
            st.markdown(html_t, unsafe_allow_html=True)
