...
st.sidebar.image("logo sta-luz.png", use_container_width=True)

st.sidebar.markdown("### Navegação")
page = st.sidebar.radio("", ["Prestação de Contas Mensal", "Fluxo de Caixa Gerencial"])
st.sidebar.markdown("---")
...
if page == "Prestação de Contas Mensal":
    ...
    with colA:
        ...
        if not df_receitas.empty:
            ...
            df_rec_grp_pie = df_rec_grp.copy()
            threshold_rec = 0.03 * total_receitas
            mask_rec = df_rec_grp_pie['amount'] < threshold_rec
            if mask_rec.any():
                outros_val = df_rec_grp_pie.loc[mask_rec, 'amount'].sum()
                df_rec_grp_pie = df_rec_grp_pie[~mask_rec]
                df_rec_grp_pie = pd.concat([df_rec_grp_pie, pd.DataFrame([{'category_name': 'Outros', 'amount': outros_val}])], ignore_index=True)
                
            df_rec_grp_pie = df_rec_grp_pie.rename(columns={'category_name': 'Categoria', 'amount': 'Valor'})
            
            fig_rec = px.pie(df_rec_grp_pie, values='Valor', names='Categoria', hole=0.4,
            ...
            df_rec_table = df_receitas.groupby('category_name').agg(
                Quantidade=('id', 'count'),
                Valor=('amount', 'sum')
            ).reset_index()
            ...
            df_rec_table['Porcentagem'] = (df_rec_table['Valor'] / total_receitas * 100).apply(lambda x: f"{x:.2f}%")
            df_rec_table['Valor'] = df_rec_table['Valor'].apply(lambda x: f"R$\xa0{x:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
            df_rec_table = df_rec_table[['category_name', 'Porcentagem', 'Quantidade', 'Valor']].rename(columns={'category_name': 'Categoria'})
            df_rec_table.set_index('Categoria', inplace=True)
            st.table(df_rec_table)
        else:
    ...
    with colB:
        ...
        if not df_despesas.empty:
            ...
            fig_desp = px.pie(df_desp_grp_pie, values='Valor', names='Categoria', hole=0.4,
                              color_discrete_sequence=px.colors.qualitative.Pastel)
            ...
            df_desp_table = df_despesas.groupby('category_name').agg(
                Quantidade=('id', 'count'),
                Valor=('amount', 'sum')
            ).reset_index()
            # Sort by numeric Value descending
            ...
            df_desp_table['Porcentagem'] = (df_desp_table['Valor'] / total_despesas * 100).apply(lambda x: f"{x:.2f}%")
            df_desp_table['Valor'] = df_desp_table['Valor'].apply(lambda x: f"R$\xa0{x:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
            df_desp_table = df_desp_table[['category_name', 'Porcentagem', 'Quantidade', 'Valor']].rename(columns={'category_name': 'Categoria'})
            df_desp_table.set_index('Categoria', inplace=True)
            st.table(df_desp_table)
        else:
...
# Tabela Dinâmica Detalhada
        ...
        def format_html_cell(val, prev_val, row_type):
            ...
            if prev_val is not None:
                if val == 0 and prev_val == 0:
                    ...
                    indicator = " <span style='color:#c0392b; font-weight:bold;'>↓</span>"
                else:
                    indicator = " <span style='color:gray; font-weight:bold;'>-</span>"
                    
            val_str = f"R$\xa0{val:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
            return f"<span style='color:{color};'>{val_str}</span>{indicator}"
            
        def build_row(cat_name, row_series, row_type, is_bold=False):
            html = f"<tr style='border-bottom: 1px solid #ddd;'>"
            weight = "bold" if is_bold else "normal"
            html += f"<td style='padding: 8px; font-weight: {weight};'>{cat_name}</td>"
            prev_val = None
            for col in ordered_cols:
                val = row_series.get(col, 0)
                cell_html = format_html_cell(val, prev_val, row_type)
        ...
        # Table Header
        html_table += "<tr style='border-bottom: 2px solid #aaa;'>"
        html_table += "<th style='padding: 8px;'>Categoria</th>"
        for col in ordered_cols:
        ...
        for cat in sorted(df_rec_pivot.index):
            html_table += build_row(f"&nbsp;&nbsp;&nbsp;&nbsp;{cat}", df_rec_pivot.loc[cat], "REC", is_bold=False)
            
        # Despesas
        html_table += build_row("DESPESAS (TOTAL)", total_desp, "DESP", is_bold=True)
        for cat in sorted(df_desp_pivot.index):
            html_table += build_row(f"&nbsp;&nbsp;&nbsp;&nbsp;{cat}", df_desp_pivot.loc[cat], "DESP", is_bold=False)
