...
<style>
@media print {
    /* Scale the page down slightly so nothing bleeds off A4 */
    html, body { zoom: 0.75; }
    ...
    * { color: black !important; }
    ...
    /* Fix columns layout for printing */
    div[data-testid="stHorizontalBlock"] { flex-wrap: wrap !important; display: flex !important; }
    div[data-testid="column"] { flex: 0 0 100% !important; min-width: 100% !important; max-width: 100% !important; margin-bottom: 30px !important; }
    
    /* BULLETPROOF fix for truncated metrics */
    ...
    /* Fix Plotly chart sizes */
    .stPlotlyChart { max-height: 400px !important; overflow: hidden !important; page-break-inside: avoid; }
    
    /* Fix HTML tables bleeding */
    div[style*="overflow-x: auto"], div[style*="overflow-x:auto"], div[style*="overflow"] { overflow: visible !important; }
    table { width: 100% !important; max-width: 100% !important; page-break-inside: avoid; color: black !important; table-layout: auto !important; }
    th, td { white-space: normal !important; word-wrap: break-word !important; color: black !important; font-size: 14px !important; padding: 4px !important; }
    ...
    /* Hide the first stHorizontalBlock (which contains the metrics) during print */
    div[data-testid="stHorizontalBlock"]:nth-of-type(1) { display: none !important; }
}
...
if page == "Prestação de Contas Mensal":
    ...
    <div class="print-only">
        <div style="font-size: 24px; line-height: 2.5; margin-top: 40px; margin-left: 20px;">
            <div><b>Saldo Anterior:</b> R$ {saldo_anterior:,.2f}</div>
            <div><b>Total Recebimentos:</b> R$ {total_receitas:,.2f} <span style="font-size: 16px;">({len(df_receitas)} transações)</span></div>
            <div><b>Total Despesas:</b> R$ {-total_despesas:,.2f} <span style="font-size: 16px;">({len(df_despesas)} transações)</span></div>
            <div><b>Balanço no Período:</b> R$ {balanco:,.2f}</div>
            <div><b>Saldo Final:</b> R$ {saldo_final:,.2f}</div>
        </div>
    </div>
    """.replace(",", "X").replace(".", ",").replace("X", ".")
    st.markdown(html_print_resumo, unsafe_allow_html=True)
